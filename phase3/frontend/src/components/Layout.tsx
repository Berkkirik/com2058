...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      >
        <div className="max-w-[1200px] mx-auto h-12 flex items-center px-6 gap-8">
          <Link to="/" className="flex items-center gap-2 no-underline">
            <span className="inline-block h-6 w-6 rounded-[6px] bg-gradient-to-br from-white to-white/60 flex items-center justify-center">
              <svg viewBox="0 0 16 16" className="h-3 w-3 text-black">
          ...
          <nav className="hidden md:flex items-center gap-1 text-[12px]">
            ...
            <a href="/docs" target="_blank" rel="noreferrer" className="px-3 py-1.5 text-white/90 hover:text-white transition-colors duration-200 no-underline">
              API Docs
            </a>
          </nav>
      ...
      <footer className="section-light py-12 border-t border-black/5">
...
function NavItem({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `px-3 py-1.5 rounded-md transition-colors duration-200 no-underline ${
          isActive
